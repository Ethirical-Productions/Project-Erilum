...
bot.once('ready', () => {
    ...
    bot.user.setActivity("Being developed by Ethirical Productions ©").then(r => null);
...
async function doJSONUpdate(server) {
    if (fs.existsSync("./Project Erilum JS Files/ServerConfigFiles/" + server.id.toString() + ".json")) {
        let template = JSON.parse(fs.readFileSync("./Project Erilum JS Files/ServerConfigFiles/defaultTemplate.json").toString());
        let serverJSON = JSON.parse(fs.readFileSync("./Project Erilum JS Files/ServerConfigFiles/" + server.id.toString() + ".json").toString());
        if (server.available) {
            serverJSON = {...template, ...serverJSON};
